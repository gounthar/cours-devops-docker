...
image::fichiers.png[background, size=cover, position=left]
...
image::docker-commands-table.png[background, size=contain, position=center]
...
[.notes]
--
Les trois cartes sont produites par `scripts/gen_naming_card.py`, appelé par
`make build`. Elles remplacent trois PNG exportés d'un PowerPoint, qui listaient
un conteneur `centos` (#550). Pour les régénérer à la main :
...
[.notes]
--
Dans le contexte de l'utilisation de la commande docker container inspect ou docker container ls, l'une des principales différences entre ENTRYPOINT et CMD réside dans leur rôle et leur impact sur la visibilité des informations.

1. **ENTRYPOINT** :
Lorsque vous utilisez docker container inspect ou docker container ls pour examiner un conteneur qui utilise ENTRYPOINT, vous verrez que ENTRYPOINT apparaît en tant que commande principale du conteneur. Cela signifie que l'entrée principale de l'application, généralement un exécutable ou un script, est affichée.
...
Si un ENTRYPOINT est défini, la commande spécifiée en utilisant docker container run est ignorée.
2. **CMD** :
Lorsque vous utilisez docker container inspect ou docker container ls pour examiner un conteneur qui utilise CMD, vous verrez que la commande CMD est répertoriée sous forme d'argument pour /bin/sh -c, ce qui signifie qu'elle est exécutée par le shell par défaut du conteneur.
Les options et les arguments passés en ligne de commande lors du lancement du conteneur remplacent la commande CMD.
Vous pouvez également ajouter des arguments supplémentaires à la commande CMD en utilisant docker container run.
...
[source,bash]
----
wget -O sample.war https://github.com/gounthar/tp07-sample/releases/download/v2026.09.07-083440/sample-2026.09.07-083440.war tomcat:11.0.26-jdk25-temurin-noble
----
...
[source,bash]
----
docker image build -t spring-boot-app --file Dockerfile.ugly .
----
