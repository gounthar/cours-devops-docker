...
http {
    ...
    upstream backend {
        server app-server:8080;  # ✅ Fonctionne !
    }
...
[TIP]
====
**127.0.0.11** est l'adresse du **DNS interne Docker** dans tous les réseaux Docker.
====
...
| upstream block avec noms Docker
| ✅ OUI
| NGINX résout au démarrage

| upstream avec variables (`$backend_host`)
| ✅ OUI
| Résolution dynamique requise
