...
[.notes]
--
**ALTERNATIVES POUR LA GESTION DES SECRETS - GUIDE DÉTAILLÉ :**
...
Avantages :
- ✅ Natif Docker, pas de service externe
- ✅ Crypté au repos et en transit
...
- ❌ Moins de fonctionnalités que Vault

Exemple d'utilisation :
```bash
echo "mon_secret_123" | docker secret create db_password -
docker service create --secret db_password postgres
# Le secret sera disponible dans /run/secrets/db_password
```

Quand l'utiliser ?
- Environnements Docker Swarm en production
- Petites/moyennes infrastructures sans besoin d'orchestration complexe
- Alternative simple à Vault pour débuter

**2. VAULT (HashiCorp)**

Qu'est-ce que c'est ?
- Solution d'entreprise dédiée à la gestion de secrets
...
- ✅ Rotation automatique des secrets (DB credentials, API keys)
- ✅ Audit trail complet (qui a accédé à quoi, quand)
- ✅ Secrets dynamiques (génère des credentials temporaires)
- ✅ Intégration avec cloud providers (AWS, Azure, GCP)
- ✅ Fonctionne avec Docker Compose, Swarm, Kubernetes

Inconvénients :
- ❌ Infrastructure supplémentaire à gérer (serveur Vault à maintenir)
- ❌ Courbe d'apprentissage plus élevée
- ❌ Coût (version enterprise pour features avancées)

Exemple d'utilisation :
```bash
# Application lit le secret depuis Vault via API
vault kv get -field=password secret/database/prod
```

Quand l'utiliser ?
- Grandes organisations avec de nombreux secrets
- Exigences de compliance strictes (audit, rotation obligatoire)
- Multi-cloud ou infrastructure hybride
- Besoin de secrets dynamiques (ex: credentials DB temporaires)

**3. CLOUD PROVIDERS (AWS Secrets Manager, Azure Key Vault, GCP Secret Manager)**
...
Avantages :
- ✅ Entièrement managé (pas de serveur à maintenir)
- ✅ Intégration native avec EC2, Lambda, AKS, etc.
- ✅ Rotation automatique pour certains services (RDS, etc.)
- ✅ Haute disponibilité garantie par le provider
- ✅ Audit via CloudTrail / Azure Monitor / Cloud Logging

Inconvénients :
- ❌ Vendor lock-in (dépendance au cloud provider)
- ❌ Coût à l'usage (par secret, par requête)
- ❌ Moins flexible que Vault pour du multi-cloud

Exemples :
- **AWS Secrets Manager** : rotation auto des credentials RDS/Aurora
- **Azure Key Vault** : intégration avec Azure AD, HSM hardware
- **GCP Secret Manager** : intégration avec Cloud Run, GKE

Quand l'utiliser ?
- Infrastructure déjà sur un cloud public (AWS, Azure, GCP)
- Pas envie de gérer un serveur Vault
- Budget pour services managés
- Besoin d'intégration native avec services cloud

**4. VARIABLES D'ENVIRONNEMENT DU SHELL**

Qu'est-ce que c'est ?
- Passer les secrets via variables d'environnement du shell au lancement
- Exemple : `DB_PASSWORD=secret123 docker compose up`

Avantages :
- ✅ Très simple, pas de fichier .env à gérer
- ✅ Secrets restent dans la session shell (pas versionnés)
- ✅ Utile pour CI/CD (secrets injectés par le pipeline)

Inconvénients :
- ❌ Visible dans `ps aux` (autres utilisateurs peuvent voir)
- ❌ Reste dans l'historique shell (~/.bash_history)
- ❌ Moins pratique pour plusieurs secrets
...
- CI/CD (GitLab CI, GitHub Actions injectent des variables)
- Scripts d'automatisation ponctuels

**TABLEAU COMPARATIF POUR LES ÉTUDIANTS :**
...
| Vault                 | Élevée     | Variable  | Grandes organisations      |
| Cloud Secrets Manager | Moyenne    | Payant    | Déjà sur AWS/Azure/GCP     |
| Env variables shell   | Faible     | Gratuit   | Dev local, CI/CD           |

**POUR LE TP SECUREVOTE :**
- Phase 1-2-3 : fichiers .env (simple, pédagogique)
- Si temps bonus : mentionner Docker Secrets comme évolution possible
- Discussion : "Dans votre entreprise, qu'utiliseriez-vous pour 100+ secrets ?" → Vault ou cloud provider

**MESSAGE CLÉ :**
"Le fichier .env est parfait pour débuter et les petits projets.
En entreprise, pour des secrets critiques et nombreux, investissez dans Vault ou les services cloud managés."
--
