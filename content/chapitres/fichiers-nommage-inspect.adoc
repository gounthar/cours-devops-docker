...
[source,bash]
----
docker run --name my-nginx -v /fully/qualified/path/on/my/computer/:/usr/share/nginx/html:ro -d -p 8000:80 nginx:1.29.6-alpine3.23
----
...
[source,bash]
----
# Used to run a Docker container with the name "tomcat-container" using the "tomcat:11.0.20-jdk25-temurin-noble" image. The container is
