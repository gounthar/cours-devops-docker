...
[source,bash]
----
docker run --name my-nginx -v /fully/qualified/path/on/my/computer/:/usr/share/nginx/html:ro -d -p 8000:80 nginx:1.31.5-alpine3.24
----
...
[source,bash]
----
# Used to run a Docker container with the name "tomcat-container" using the "tomcat:11.0.25-jdk25-temurin-noble" image. The container is
...
# background.
docker run -d -p 8080:8080 --name tomcat-container -v /fully/qualified/path/on/my/computer/sample.war:/usr/local/tomcat/webapps/sample.war tomcat:11.0.25-jdk25-temurin-noble
----
