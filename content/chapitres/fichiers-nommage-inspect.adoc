...
[%step]
* Récupérer un war sur GitHub et le déployer dans un container `tomcat (https://github.com/gounthar/tp07-sample/releases/download/v2026.06.02-100445/sample-2026.06.02-100445.war)`
[%step]
...
[source,bash]
----
docker run --name my-nginx -v /fully/qualified/path/on/my/computer/:/usr/share/nginx/html:ro -d -p 8000:80 nginx:1.31.2-alpine3.23
----
...
[source,bash]
----
# Used to run a Docker container with the name "tomcat-container" using the "tomcat:11.0.24-jdk25-temurin-noble" image. The container is
...
# background.
docker run -d -p 8080:8080 --name tomcat-container -v /fully/qualified/path/on/my/computer/sample.war:/usr/local/tomcat/webapps/sample.war tomcat:11.0.24-jdk25-temurin-noble
----
