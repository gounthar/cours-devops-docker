...
Idée clé : le secret n'entre jamais dans l'environnement d'exécution. C'est un modèle différent des cinq autres, pas une sixième ligne du tableau.
...
TIP: Le proxy empêche le secret de **sortir**, pas d'**être utilisé**.

[.notes]
--
Chaque point a été mesuré le 2026-09-24 :

1. Usage : la sandbox ne connaît pas la valeur, mais toute requête vers httpbin.org part avec. Un agent compromis peut donc agir avec le jeton (créer une issue, pousser du code...) sur le service déclaré. Ce qu'il ne peut pas, c'est emporter la valeur ailleurs.
