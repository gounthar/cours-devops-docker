...
When you run docker container create, Docker creates a container based on the specified image and prepares it to run, but it remains in a stopped state.
You can then start the container using the `docker container start` command.

Here's the basic syntax of the docker container create command:
...
# Supprimer les containers
docker container stop second-clock
second-clock

docker container rm second-clockl
second-clock
# Supprimer l'image (pour de vrai cette fois)
docker image rm jpetazzo/clock
