...
* L'ISO se télécharge et s'installe sans licence{nbsp}: un type{nbsp}1 que vous pouvez vraiment essayer, sur une machine que vous lui **dédiez** — elle ne fera plus que ça
--
...
Insister sur « dédiez » : un hôte XCP-ng prend la machine entière. La documentation est explicite — « An XCP-ng server is dedicated entirely to running XCP-ng and hosting VMs. It is not used for other applications. » Il faut aussi VT-x ou AMD-V activé, 2 Go de RAM au minimum et 46 Go de disque (https://docs.xcp-ng.org/installation/requirements/, consultée le 2026-09-10). Ne pas laisser un étudiant lancer l'ISO sur le portable qui lui sert à suivre le cours.

Le logo `xcpng.svg` est déjà dans le dépôt, il sert dans `welcome.adoc`.
--

[.columns]
== On ne pilote pas XCP-ng à la main

[.column]
--
* Sur un hôte XCP-ng, ni bureau ni fenêtre{nbsp}: la machine ne montre qu'une console
* L'administration passe par un outil **séparé**, qui parle à l'hôte par le réseau
--

[.column]
--
* **Xen Orchestra** est celui de XCP-ng{nbsp}: visualiser, gérer, sauvegarder et déléguer un parc, depuis un navigateur
* Sauvegardes complètes ou incrémentales, réplication, instantanés tournants — qui ne remplacent pas une sauvegarde
...
Ne pas en faire une règle générale des type 1 : Proxmox VE et ESXi servent leur propre interface web depuis l'hôte. La console seule est le cas de XCP-ng, pas celui de toute la famille. C'est pour ça que la slide dit « sur un hôte XCP-ng » et pas « un hyperviseur de type 1 ».

Sur les instantanés tournants : ils restent sur le même stockage que la VM. Pratiques pour revenir en arrière vite, inutiles le jour où c'est la baie qui tombe. La nuance est à l'écran parce que confondre les deux est une erreur qu'on retrouve ensuite sur les volumes Docker.
...
* Dans les TP, on ne travaillera qu'à l'étage du haut
--
