...
# Stage 2: Runtime
FROM tomcat:11-jre17-temurin-noble
...
[source,bash]
----
docker manifest inspect tomcat:11-jre17-temurin-noble
----
...
L'image de base `tomcat:11-jre17-temurin-noble` pèse 414 MB ; le Dockerfile
...
command:
  docker ps --filter name=reverse-proxy --filter status=running --quiet:
    exit-status: 0
    stdout:
      - "/[0-9a-f]{12}/"
...
Ces trois blocs supposent que Goss voit le démon Docker : le conteneur qui
l'exécute doit avoir la CLI Docker et `/var/run/docker.sock` monté.
...
[source,bash]
----
# Arrêter un des trois services, ou viser un nom inexistant, puis relancer
