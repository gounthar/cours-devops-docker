...
image::2073552164-image2.png[height=550]
...
2. ✅ **Tester** : `docker container run --rm image commande` pour vérifier
...
[TIP]
====
**Règle d'or** : `docker container run --rm mon-image commande`
...
[.column]
image::when.png[height=500]
[.column]
video::gitlab-logs.speed.very.small.mp4[height=500,options="autoplay,loop,nocontrols"]
...
[.column]
image::gitlab-login.png[height=500]
...
[source,yaml]
----
include::../code-samples/compose/cours/docker-compose.yml[tags=**]
----
...
[source,yaml]
----
include::../code-samples/compose/cours/docker-compose.yml[tags="entete,ancre"]
----
...
[source,yaml]
----
include::../code-samples/compose/cours/docker-compose.yml[tags=qrcode]
----

[%auto-animate]
== Autre étude de cas 📚🔍

Le service qui construit les slides à partir de `asciidoc` avec `reveal.js`.
[%step]
[source,yaml]
----
include::../code-samples/compose/cours/docker-compose.yml[tags=build]
----
...
[source,yaml]
----
include::../code-samples/compose/cours/docker-compose.yml[tags=serve]
----
...
[source,yaml]
----
include::../code-samples/compose/cours/docker-compose.yml[tags=pdf]
----

== Pourquoi une image construite, et pas l'image publiée ? 📄

L'image publiée `ghcr.io/astefanutti/decktape` ne suffit plus telle quelle.
[%step]
* Le PDF passe par deux étapes : decktape rend les diapositives en 2048x1536, puis `ghostscript` réduit le fichier. L'image publiée n'a pas `ghostscript`.
* Chromium ne démarre dans le conteneur qu'avec `--no-sandbox` et `--disable-gpu`. Ces options vivent dans `entrypoint.sh`, d'où l'absence de `command:` dans le service.
* L'image ne déclare un `HOME` que pour l'uid 1000. Avec un autre uid, Chromium échoue sur `chrome_crashpad_handler: --database is required` : d'où `HOME=/tmp`.
[%step]
Un petit `Dockerfile` `FROM ghcr.io/astefanutti/decktape` règle les deux premiers points ; Compose le construit avec `build:`.
...
[source,yaml]
----
include::../code-samples/compose/cours/docker-compose.yml[tags=ancre]
----
...
[%step]
image::multi-layered-cake-removebg-preview.png[height=100]
--
[.column.is-one-third]
--
[%step]
...
[source,yaml]
----
include::../code-samples/compose/cours/docker-compose.yml[tags=environnement]
----
--
[.column]
--
Trois variables d'environnement sont définies pour le conteneur Docker : `PRESENTATION_URL`, `REPOSITORY_URL` et `SOURCE_FILE`.
...
* Dans notre cas, aucune valeur par défaut n'est spécifiée. Docker Compose n'échoue pas pour autant : il émet un avertissement, substitue une *chaîne vide*, et continue.
...
[source,yaml]
----
include::../code-samples/compose/cours/docker-compose.yml[tags=utilisateur]
----
[%step]
...
* Si `CURRENT_UID` n'est pas défini, Docker Compose *n'échoue pas* : même avertissement, même chaîne vide. `user:` reçoit une valeur vide.
* Le conteneur démarre donc *en root* — exactement l'inverse de ce que cette ligne cherchait à obtenir.

== 🛡️ Rendre une variable vraiment obligatoire
...
* `+${VAR:?message}+` échoue pour de bon, dès `docker compose config` : le message s'affiche et le code de sortie est 1.
* Rien ne démarre, donc rien ne démarre en root.
* C'est la forme à écrire quand la valeur est obligatoire.
* C'est celle qu'utilise le `docker-compose.yml` à la racine de ce dépôt : sans `CURRENT_UID` (que `make` fournit), il s'arrête au lieu de construire en root.
...
[source,makefile]
----
include::../code-samples/compose/cours/Makefile[lines=8..15]
----
[%step]
* Voilà d'où vient le `CURRENT_UID` que `docker-compose.yml` consomme quelques diapos plus haut.
* Compose ne le calcule pas, et ne se plaint pas non plus s'il manque : c'est au `Makefile` de le poser.
* `$(shell id -u):$(shell id -g)` le lit à chaque appel, au lieu de le figer dans `.env`.
...
[source,makefile]
----
include::../code-samples/compose/cours/Makefile[lines=17..20]
----
...
[source,makefile]
----
include::../code-samples/compose/cours/Makefile[lines=22..39]
----
...
[source,makefile]
----
include::../code-samples/compose/cours/Makefile[lines=44..50]
----
[%step]
...
image::bitmoji-whale friend.png[height=500]
