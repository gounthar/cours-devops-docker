...
[%step]
* Récupérer un war sur GitHub et le déployer dans un container `tomcat (https://github.com/gounthar/tp07-sample/releases/download/v2025.12.15-093217/sample-2025.12.15-093217.war)`
[%step]
...
[source,bash]
----
docker run --name my-nginx -v /fully/qualified/path/on/my/computer/:/usr/share/nginx/html:ro -d -p 8000:80 nginx:1.30.0-alpine3.23
----
...
# background.
docker run -d -p 8080:8080 --name tomcat-container -v /fully/qualified/path/on/my/computer/sample.war:/usr/local/tomcat/webapps/sample.war tomcat:11.0.22-jdk25-temurin-noble
----
