...
[%step]
Avec `apk add --no-cache`, l'index des paquets n'est jamais écrit dans l'image : il n'y a pas de cache à vider ensuite.
...
[NOTE]
====
Alpine n'a pas `bash`, seulement le `sh` de BusyBox. Beaucoup de scripts copiés depuis Internet échouent sur les images minimales pour cette raison : `executable file not found`.
====
...
[.column]
image::https://about.gitlab.com/images/press/press-kit-icon.svg[height=200]
[.column]
image::https://avatars.githubusercontent.com/u/27347476?s=200&v=4[height=200]
...
* Vous pouvez vérifier cela en inspectant les réseaux de chaque conteneur à l'aide de la commande `docker container inspect`.
...
rappel : `docker container inspect` et `docker image inspect` listent toutes les caractéristiques d'un conteneur ou d'une image.
...
Rassurez-vous, on nous a dit que `docker image load` est la première étape de la formule magique pour percer les secrets de son œuvre.
...
`docker image load` est la première étape de la formule magique
[%step]
Okay...
[%step]
🧙‍♂️✨
...
[%step]
La quatrième ligne, c'est l'installation de Nginx par l'infâme `yum` de Centos (déjà repéré dans le `docker image inspect`).
--
...
[%step]
La sixième ligne, c'est l' `EXPOSE` de l'image finale, qui est `80` (déjà repéré dans le `docker image inspect`).
[%step]
La septième ligne, c'est le `ENTRYPOINT` de l'image finale, qui est `/bin/sh -c /tmp/entrypoint.sh` (déjà repéré dans le `docker image inspect`).
...
[.column]
--
image::final answer.png[height=500]
--
[.column]
--
...
image::dont panic.png[height=500]
...
image::415439355-image7.png[height=500]
