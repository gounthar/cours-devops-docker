...
[%step]
* Récupérer un war sur GitHub et le déployer dans un container `tomcat (https://github.com/gounthar/tp07-sample/releases/download/v2025.10.27-172857/sample-2025.10.27-172857.war)`
[%step]
...
[source,bash]
----
# Used to run a Docker container with the name "tomcat-container" using the "tomcat:11.0.14-jdk25-temurin-noble" image. The container is
...
# background.
docker run -d -p 8080:8080 --name tomcat-container -v /fully/qualified/path/on/my/computer/sample.war:/usr/local/tomcat/webapps/sample.war tomcat:11.0.14-jdk25-temurin-noble
----
