...
[source,bash]
----
# Used to run a Docker container with the name "tomcat-container" using the "tomcat:11.0.11-jdk25-temurin-noble" image. The container is
...
# background.
docker run -d -p 8080:8080 --name tomcat-container -v /fully/qualified/path/on/my/computer/sample.war:/usr/local/tomcat/webapps/sample.war tomcat:11.0.11-jdk25-temurin-noble
----
