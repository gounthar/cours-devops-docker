...
**Résultat attendu de `docker container run hello-world` :**
```
...
[%step]
4. ✓ Commande `docker container run hello-world` s'exécute avec succès
...
**4. `docker container run hello-world` échoue :**
...
"J'essaie de lancer `docker container run nginx` mais j'obtiens l'erreur 'Cannot connect to the Docker daemon'. J'ai vérifié avec `docker --version` et ça fonctionne. Que dois-je faire ?"
